...
export class PaneSeparator implements IDestroyable {
	...
	private _className: string = 'lwc-separator-handle';
	private _classNameActive: string = 'lwc-separator-handle--active';
	...
	public constructor(chartWidget: IChartWidgetBase, topPaneIndex: number, bottomPaneIndex: number, disableResize: boolean) {
		...
		if (disableResize) {
			...
			this._handle.classList.add(this._className);
	...
	private _mouseUpEvent(event: TouchMouseEvent): void {
		...
		this._handle?.classList.remove(this._classNameActive);
