...
export class CrosshairMarksPaneView implements IUpdatablePaneView {
	...
	private _updateImpl(pane: Pane): PaneRendererMarks[] {
		...
		const serieses = this._chartModel.serieses()
			.map((datasource: ISeries<SeriesType>, index: number): [ISeries<SeriesType>, number] => [datasource, index])
			.filter((entry: [ISeries<SeriesType>, number]) => pane.dataSources().includes(entry[0]));
